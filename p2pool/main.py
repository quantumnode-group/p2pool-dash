...
import qnodecoin.p2p as qnodecoin_p2p, qnodecoin.data as qnodecoin_data
from qnodecoin import stratum, worker_interface, helper
...
class keypool():
    ...
    def paytotal(self):
        ...
        for i in xrange(len(pubkeys.keys)):
            self.payouttotal += node.get_current_txouts().get(qnodecoin_data.pubkey_hash_to_script2(pubkeys.keys[i]), 0)*1e-8
...
def main(args, net, datadir_path, merged_urls, worker_endpoint):
    try:
        ...
        @defer.inlineCallbacks
        def connect_p2p():
            # connect to qnodecoind over qnodecoin-p2p
            print '''Testing qnodecoind P2P connection to '%s:%s'...''' % (args.qnodecoind_address, args.qnodecoind_p2p_port)
            factory = qnodecoin_p2p.ClientFactory(net.PARENT)
            reactor.connectTCP(args.qnodecoind_address, args.qnodecoind_p2p_port, factory)
            def long():
                print '''    ...taking a while. Common reasons for this include all of qnodecoind's connection slots being used...'''
        ...
        if args.testnet: # establish p2p connection first if testnet so qnodecoind can work without connections
            factory = yield connect_p2p()
        
        # connect to qnodecoind over JSON-RPC and do initial getmemorypool
        url = '%s://%s:%i/' % ('https' if args.qnodecoind_rpc_ssl else 'http', args.qnodecoind_address, args.qnodecoind_rpc_port)
        print '''Testing qnodecoind RPC connection to '%s' with username '%s'...''' % (url, args.qnodecoind_rpc_username)
        qnodecoind = jsonrpc.HTTPProxy(url, dict(Authorization='Basic ' + base64.b64encode(args.qnodecoind_rpc_username + ':' + args.qnodecoind_rpc_password)), timeout=30)
        yield helper.check(qnodecoind, net)
        temp_work = yield helper.getwork(qnodecoind, net)
        
        qnodecoind_getnetworkinfo_var = variable.Variable(None)
        @defer.inlineCallbacks
        def poll_warnings():
            qnodecoind_getnetworkinfo_var.set((yield deferral.retry('Error while calling getnetworkinfo:')(qnodecoind.rpc_getnetworkinfo)()))
        yield poll_warnings()
        ...
        if args.pubkey_hash is None and args.address != 'dynamic':
            ...
            if address is not None:
                res = yield deferral.retry('Error validating cached address:', 5)(lambda: qnodecoind.rpc_validateaddress(address))()
                if not res['isvalid'] or not res['ismine']:
                    print '    Cached address is either invalid or not controlled by local qnodecoind!'
                    address = None
            
            if address is None:
                print '    Getting payout address from qnodecoind...'
                address = yield deferral.retry('Error getting payout address from qnodecoind:', 5)(lambda: qnodecoind.rpc_getaccountaddress('p2pool'))()
            
            with open(address_path, 'wb') as f:
                f.write(address)
            
            my_pubkey_hash = qnodecoin_data.address_to_pubkey_hash(address, net.PARENT)
            print '    ...success! Payout address:', qnodecoin_data.pubkey_hash_to_address(my_pubkey_hash, net.PARENT)
            print
            ...
            for i in xrange(args.numaddresses):
                address = yield deferral.retry('Error getting a dynamic address from qnodecoind:', 5)(lambda: qnodecoind.rpc_getnewaddress('p2pool'))()
                new_pubkey = qnodecoin_data.address_to_pubkey_hash(address, net.PARENT)
            ...
            for i in xrange(len(pubkeys.keys)):
                print '    ...payout %d: %s' % (i, qnodecoin_data.pubkey_hash_to_address(pubkeys.keys[i], net.PARENT),)
        ...
        node = p2pool_node.Node(factory, qnodecoind, shares.values(), known_verified, net)
        yield node.start()
        ...
        wb = work.WorkerBridge(node, my_pubkey_hash, args.donation_percentage, merged_urls, args.worker_fee, args, pubkeys, qnodecoind)
        web_root = web.get_web_root(wb, datadir_path, qnodecoind_getnetworkinfo_var, static_dir=args.web_static)
                ...
                def signedOn(self):
                    ...
                    def new_share(share):
                        ...
                        if share.pow_hash <= share.header['bits'].target and abs(share.timestamp - time.time()) < 10*60:
                            ...
                            message = '\x02%s BLOCK FOUND by %s! %s%064x' % (net.NAME.upper(), qnodecoin_data.script2_to_address(share.new_script, net.PARENT), net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash)
        ...
        def status_thread():
            ...
            while True:
                ...
                try:
                    ...
                    my_shares_per_s = sum(datum['work']/dt/qnodecoin_data.target_to_average_attempts(datum['share_target']) for datum in datums)
                    ...
                    if height > 2:
                        ...
                        paytot = 0.0
                        for i in xrange(len(pubkeys.keys)):
                            curtot = node.get_current_txouts().get(qnodecoin_data.pubkey_hash_to_script2(pubkeys.keys[i]), 0)
                            paytot += curtot*1e-8
                        ...
                        this_str += '\n Pool: %sH/s Stale rate: %.1f%% Expected time to block: %s' % (
                            ...
                            100*stale_prop,
                            math.format_dt(2**256 / node.qnodecoind_work.value['bits'].target / real_att_s),
                        )
                        
                        for warning in p2pool_data.get_warnings(node.tracker, node.best_share_var.value, net, qnodecoind_getnetworkinfo_var.value, node.qnodecoind_work.value):
                            print >>sys.stderr, '#'*40
...
def run():
    ...
    parser.add_argument('--net',
        help='use specified network (default: qnodecoin)',
        action='store', choices=sorted(realnets), default='qnodecoin', dest='net_name')
    ...
    parser.add_argument('-a', '--address',
        help='generate payouts to this address (default: <address requested from qnodecoind>), or (dynamic)',
        type=str, action='store', default=None, dest='address')
    parser.add_argument('-i', '--numaddresses',
        help='number of qnodecoin auto-generated addresses to maintain for getwork dynamic address allocation',
    ...
    worker_group.add_argument('-f', '--fee', metavar='FEE_PERCENTAGE',
        help='''charge workers mining to their own qnodecoin address (by setting their miner's username to a qnodecoin address) this percentage fee to mine on your p2pool instance. Amount displayed at http://127.0.0.1:WORKER_PORT/fee (default: 0)''',
        type=float, action='store', default=0, dest='worker_fee')
    
    qnodecoind_group = parser.add_argument_group('qnodecoind interface')
    qnodecoind_group.add_argument('--qnodecoind-config-path', metavar='QNODECOIND_CONFIG_PATH',
        help='custom configuration file path (when qnodecoind -conf option used)',
        type=str, action='store', default=None, dest='qnodecoind_config_path')
    qnodecoind_group.add_argument('--qnodecoind-address', metavar='QNODECOIND_ADDRESS',
        help='connect to this address (default: 127.0.0.1)',
        type=str, action='store', default='127.0.0.1', dest='qnodecoind_address')
    qnodecoind_group.add_argument('--qnodecoind-rpc-port', metavar='QNODECOIND_RPC_PORT',
        help='''connect to JSON-RPC interface at this port (default: %s <read from qnodecoin.conf if password not provided>)''' % ', '.join('%s:%i' % (name, net.PARENT.RPC_PORT) for name, net in sorted(realnets.items())),
        type=int, action='store', default=None, dest='qnodecoind_rpc_port')
    qnodecoind_group.add_argument('--qnodecoind-rpc-ssl',
        help='connect to JSON-RPC interface using SSL',
        action='store_true', default=False, dest='qnodecoind_rpc_ssl')
    qnodecoind_group.add_argument('--qnodecoind-p2p-port', metavar='QNODECOIND_P2P_PORT',
        help='''connect to P2P interface at this port (default: %s <read from qnodecoin.conf if password not provided>)''' % ', '.join('%s:%i' % (name, net.PARENT.P2P_PORT) for name, net in sorted(realnets.items())),
        type=int, action='store', default=None, dest='qnodecoind_p2p_port')
    
    qnodecoind_group.add_argument(metavar='QNODECOIND_RPCUSERPASS',
        help='qnodecoind RPC interface username, then password, space-separated (only one being provided will cause the username to default to being empty, and none will cause P2Pool to read them from qnodecoin.conf)',
        type=str, action='store', default=[], nargs='*', dest='qnodecoind_rpc_userpass')
    ...
    if len(args.qnodecoind_rpc_userpass) > 2:
        parser.error('a maximum of two arguments are allowed')
    args.qnodecoind_rpc_username, args.qnodecoind_rpc_password = ([None, None] + args.qnodecoind_rpc_userpass)[-2:]
    
    if args.qnodecoind_rpc_password is None:
        conf_path = args.qnodecoind_config_path or net.PARENT.CONF_FILE_FUNC()
        if not os.path.exists(conf_path):
            parser.error('''qnodecoin configuration file not found. Manually enter your RPC password.\r\n'''
                '''If you actually haven't created a configuration file, you should create one at %s with the text:\r\n'''
                '''\r\n'''
                '''server=1\r\n'''
                '''rpcpassword=%x\r\n'''
                '''\r\n'''
                '''Keep that password secret! After creating the file, restart qnodecoin.''' % (conf_path, random.randrange(2**128)))
        ...
        for conf_name, var_name, var_type in [
            ('rpcuser', 'qnodecoind_rpc_username', str),
            ('rpcpassword', 'qnodecoind_rpc_password', str),
            ('rpcport', 'qnodecoind_rpc_port', int),
            ('port', 'qnodecoind_p2p_port', int),
        ]:
        ...
        if 'rpcssl' in contents and contents['rpcssl'] != '0':
                args.qnodecoind_rpc_ssl = True
        if args.qnodecoind_rpc_password is None:
            parser.error('''qnodecoin configuration file didn't contain an rpcpassword= line! Add one!''')
    
    if args.qnodecoind_rpc_username is None:
        args.qnodecoind_rpc_username = ''
    
    if args.qnodecoind_rpc_port is None:
        args.qnodecoind_rpc_port = net.PARENT.RPC_PORT
    
    if args.qnodecoind_p2p_port is None:
        args.qnodecoind_p2p_port = net.PARENT.P2P_PORT
    ...
    if args.address is not None and args.address != 'dynamic':
        try:
            args.pubkey_hash = qnodecoin_data.address_to_pubkey_hash(args.address, net.PARENT)
