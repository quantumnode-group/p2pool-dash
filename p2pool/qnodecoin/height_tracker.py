...
class HeaderWrapper(object):
    ...
    def from_header(cls, header):
        return cls(qnodecoin_data.hash256(qnodecoin_data.block_header_type.pack(header)), header['previous_block'])
...
@defer.inlineCallbacks
def get_height_rel_highest_func(qnodecoind, factory, best_block_func, net):
    if '\ngetblock ' in (yield deferral.retry()(qnodecoind.rpc_help)()):
        @deferral.DeferredCacher
        @defer.inlineCallbacks
        def height_cacher(block_hash):
            try:
                x = yield qnodecoind.rpc_getblock('%x' % (block_hash,))
